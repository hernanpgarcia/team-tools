...
def check_python():
    ...
    try:
        ...
        if version.major == 3 and version.minor >= 8:
            print_success(
                f"Python {version.major}.{version.minor}.{version.micro} is installed"
            )
            return True
        else:
            print_error(
...
def setup_virtual_environment():
    ...
    # Upgrade pip
    if not run_command(
        f"{activate_cmd}python -m pip install --upgrade pip", "Upgrade pip"
    ):
        return False

    # Install requirements
    if not run_command(
        f"{activate_cmd}pip install -r requirements.txt", "Install main requirements"
    ):
        return False

    # Install test requirements
    if not run_command(
        f"{activate_cmd}pip install -r requirements-test.txt",
        "Install test requirements",
    ):
        return False
...
def run_tests():
    ...
    # Run quick test
    if not run_command(
        f"{activate_cmd}python quick_test.py", "Run quick functionality test"
    ):
        return False
