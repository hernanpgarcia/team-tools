...
class TestFixedHorizonCalculator:
    ...
    def test_one_sided_vs_two_sided(self):
        ...
        assert (
            result_one_sided["sample_size_per_group"]
            < result_two_sided["sample_size_per_group"]
        )
    ...
    def test_negative_baseline_std(self):
        ...
        with pytest.raises(
            ValueError, match="Baseline standard deviation must be positive"
        ):
            calculate_sample_size(
                baseline_mean=100,
                baseline_std=-5,
    ...
    @pytest.mark.parametrize(
        "baseline_mean,baseline_std,improvement_type,improvement_value,power,alpha",
        [
