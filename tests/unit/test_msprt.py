...
class TestMSPRTCalculator:
    ...
    def test_valid_known_std(self):
        ...
        assert result['std_method'] == "Known standard deviation"
    ...
    def test_valid_estimated_std(self):
        ...
        assert result is not None
        assert result['std_method'] == "Estimated standard deviation (use Welch's t-test)"
    ...
    def test_unknown_std(self):
        ...
        assert result is not None
        assert result['std_method'] == "Unknown standard deviation (robust estimation)"
        assert result['use_t_test'] == True
        assert result['baseline_std'] == 50  # Conservative estimate
    ...
    def test_efficiency_gain_calculation(self):
        ...
        expected_gain = (result['max_n'] - result['expected_n_h1']) / result['max_n'] * 100
    ...
    @pytest.mark.parametrize("baseline_mean,std_known,baseline_std,improvement_type,improvement_value,alpha,beta,max_n,min_n", [
    ...
    def test_various_parameter_combinations(self, baseline_mean, std_known, baseline_std, improvement_type, improvement_value, alpha, beta, max_n, min_n):
